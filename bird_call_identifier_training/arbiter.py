...
def build_feature_extractor_arbiter_with_value(mfcc_timesteps, mfcc_features, latent_dim, num_experts):
    ...
    x = keras.layers.DepthwiseConv2D((3, 3), strides=(2, 2), padding='same', depthwise_regularizer=keras.regularizers.l2(1e-4))(x)
    x = keras.layers.BatchNormalization()(x)
    x = keras.layers.Activation('swish')(x)
    x = keras.layers.Conv2D(24, (1, 1), padding='same', kernel_regularizer=keras.regularizers.l2(1e-4))(x)
    x = keras.layers.BatchNormalization()(x)
    x = keras.layers.Activation('swish')(x)
    ...
    x = keras.layers.BatchNormalization()(x)
    x = keras.layers.Activation('swish')(x)
    x = keras.layers.Conv2D(40, (1, 1), padding='same', kernel_regularizer=keras.regularizers.l2(1e-4))(x)
    x = keras.layers.BatchNormalization()(x)
    x = keras.layers.Activation('swish')(x)
    x = keras.layers.Dropout(0.3)(x)
    ...
    x = keras.layers.BatchNormalization()(x)
    x = keras.layers.Activation('swish')(x)
    x = keras.layers.Conv2D(80, (1, 1), padding='same', kernel_regularizer=keras.regularizers.l2(1e-4))(x)
    x = keras.layers.BatchNormalization()(x)
    x = keras.layers.Activation('swish')(x)
...
def build_feature_extractor_student_arbiter_with_value(mfcc_timesteps, mfcc_features, latent_dim, num_experts):
    num_actions = num_experts + 1
    inputs = keras.layers.Input(shape=(mfcc_timesteps, mfcc_features))
